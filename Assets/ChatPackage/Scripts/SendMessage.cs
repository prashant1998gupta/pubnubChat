...
public class SendMessage : MonoBehaviour {
    ...
    void Start()
    {
        ...
        pnConfiguration.PublishKey = "pub-c-f425597d-dc0f-48d7-b5e2-2e4595f9317c";
        pnConfiguration.SubscribeKey = "sub-c-8662494a-bd1a-11eb-9c3c-fe487e55b6a4";
        ...
        pubnub.FetchMessages()
            ...
            if (status.Error)
            {
                Debug.Log( $" FetchMessages Error: {status.StatusCode} { status.ErrorData} {status.Category}");
            }
            else
            {
                foreach (KeyValuePair<string, List<PNMessageResult>> kvp in result.Channels)
                {
                        Debug.Log($"this is no of chanel {result.Channels.Count}");

                    foreach (PNMessageResult pnMessageResult in kvp.Value)
                    {
                            Debug.Log("this is just loop count");
